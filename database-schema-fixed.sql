-- ScoreLeague Database Schema for Supabase (CORRECTED VERSION)
...
CREATE POLICY "Users can view own bet slip items" ON public.bet_slip_items
    FOR SELECT USING (auth.uid() IN (SELECT user_id FROM public.bet_slips WHERE id = bet_slip_id));

CREATE POLICY "Users can insert own bet slip items" ON public.bet_slip_items
    FOR INSERT WITH CHECK (auth.uid() IN (SELECT user_id FROM public.bet_slips WHERE id = bet_slip_id));
